...
3. Décompresser le fichier à la racine du serveur :
     Utilisez la commande ```bash $sudo apt install unzip``` ensuite ```bash $sudo unzip siteweb.zip -d /home/debian && sudo mv /home/debian/siteweb/* /home/debian/ && sudo rm -r /home/debian/siteweb ```
...
5. Lancer Docker avec docker-compose :
      Utilisez la commande ```bash $sudo docker-compose up -d``` pour démarrer les conteneurs définis dans le fichier docker-compose.yml. Assurez-vous d'exécuter cette commande dans le répertoire contenant le fichier docker-compose.yml. (```bash $sudo docker-compose down ``` pour arreter les conteneurs) 
